...
class Downloader {
    ...
    queue: string[];
    handler: (err) => void;
    constructor(cacheRoot: string, hostUri: string, resources: string[], handler: (err) => void) {
...
export class CachingProxy {
    ...
    constructor(port: number, cacheRoot: string, hostUri: string) {
        this.port = port;
        this.cacheRoot = cacheRoot;
        this.hostUri = hostUri;
    ...
    enQueue(resources: string[], handler: (err) => void) {
        var downloader = new Downloader(this.cacheRoot, this.hostUri, resources,
            function (err) {
                handler(err);
            });
    }

    setInstructions(arg: any, handler: (err) => void) {
